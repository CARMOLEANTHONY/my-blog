    ...
    <div class="list_card">
      ...
      <div v-if="detail.list && detail.list.length > 0" class="article_list">
        <el-row class="item_style">
          <el-col :span="5" :offset="1" class="ellipse strong h3_color">作者</el-col>
          <el-col :span="10" class="pointer ellipse strong h3_color">文章标题</el-col>
          <el-col :span="4" class="strong h3_color">发表时间</el-col>
          <el-col :span="4" class="strong h3_color">上次修改时间</el-col>
        </el-row>
        <el-row v-for="(item, index) in detail.list" :key="index" class="item_style">
          <el-col :span="5" :offset="1" v-html="highLightKeywords(searchValue, item.author_name)"></el-col>
          <el-col :span="10" class="pointer ellipse" @click.native="goDetail(item.article_id)" v-html="highLightKeywords(searchValue, item.title)"></el-col>
...
<script>
  ...
  export default {
      ...
      currentAuthorId: {
        handler: function (value) {
          ...
          this.getAll ?
            ...
            this.getArticleList(this.page, value);
        },
        immediate: true
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      // 缓存操作
      // to.meta.keepAlive = from.path == "/articles/detail" ? true : false;
    ...
    methods: {
      ...
      getUserList() {
        this.$fetch.get("/user/getList").then(res => {
          if (res.data.success) {
